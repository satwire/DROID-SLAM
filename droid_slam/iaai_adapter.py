...
class IAAIAdapter:
    def __init__(
        ...
        """
        trans_scale: scalar applied to every IAAI delta translation before
            building the SE3 prior. Used for the scale-correction ablation —
            set it to the IAAI->DROID scale from `align_iaai_droid.py`
            (`compute_iaai_to_droid_scale`) to bring IAAI's monocular-depth
            translation magnitudes onto DROID's metric scale. Rotation is
            left untouched (it is scale-free). Default 1.0 = no correction.
        """
